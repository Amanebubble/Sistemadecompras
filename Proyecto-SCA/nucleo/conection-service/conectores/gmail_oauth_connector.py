...
class GmailOAuthConnector(MailConnector):
    ...
    def conectar(self):
        ...
        self._etiqueta_id = self._obtener_o_crear_etiqueta("DTE-Procesado")

    def desconectar(self):
        # La librería de Google no requiere cierre explícito de sesión.
        pass

    def _obtener_o_crear_etiqueta(self, nombre_etiqueta):
        etiquetas = (
            self._servicio.users().labels().list(userId="me").execute().get("labels", [])
        )
        for et in etiquetas:
            if et["name"] == nombre_etiqueta:
                return et["id"]
    ...
    def listar_candidatos(self):
        candidatos = []
        query = f'has:attachment -label:"DTE-Procesado"'
    ...
    def marcar_procesado(self, mensaje: MensajeNormalizado):
        self._servicio.users().messages().modify(
            userId="me",
            id=mensaje.id_interno,
            body={"addLabelIds": [self._etiqueta_id]},
        ).execute()
